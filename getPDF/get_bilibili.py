...
def down_video(video_list, title, start_url, page):
    ...
    for i in video_list:
        ...
        # 开始下载
        if len(video_list) > 1:
            urllib.request.urlretrieve(url=i, filename=os.path.join(currentVideoPath, r'{}-{}.flv'.format(
                title, num)), reporthook=Schedule_cmd)  # 写成mp4也行  title + '-' + num + '.flv'
        else:
            urllib.request.urlretrieve(url=i, filename=os.path.join(currentVideoPath, r'{}.flv'.format(
                title)), reporthook=Schedule_cmd)  # 写成mp4也行  title + '-' + num + '.flv'
        num += 1
...
if __name__ == '__main__':
    ...
    if start.isdigit() == True:  # 如果输入的是av号
        ...
        start_url = 'https://api.bilibili.com/x/web-interface/view?aid=' + \
            re.search(r'/av(\d+)/*', start).group(1)
